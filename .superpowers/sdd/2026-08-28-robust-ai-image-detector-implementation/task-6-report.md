...
8. `PairedBatchCollator` rejects empty batches, duplicate sample IDs, non-`SourceItem` values, invalid labels, and invalid epochs. It derives one transform pixel seed from run seed, epoch, and stable sample ID, applies Task 5 transformations before preprocessing, and copies source images before passing them to the injected processor.
